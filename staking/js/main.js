function init() {
	...
	$('#investDVN').click(investTokens);
	$('#divestDVN').click(divestTokens);
	$('#partialDivestDVN').click(partialDivestTokens);
...
function investTokens() {
	...
	if (!$('#investmentAmount').val().length) {
		...
		investDVN(tokensToSpend, function() {
			// done
		});
	}
}
...
function partialDivestTokens() {
	...
	if (!$('#divestAmount').val().length) {
		...
		partialDivestDVN(tokensToWithdraw, function() {
			// done
		});
	}
}
...
function updateDVNBalance() {
	getDVNBalance(function(DVNBalance) {
		var myDVN = formatEthValue(web3.fromWei(DVNBalance, 'ether'));
		$('#myDVN').text(myDVN);
	});
}
...
function updateBalanceSurplus() {
	getBalance(function(balance) {
		...
		getSurplus(function(surplus) {
			...
			$('#surplusDVNPercent').text('(' + (surplus > 0 ? '+' : '') + (100 * surplus / totalWithdrawable).toFixed(1) + '%)');
		});
	});
}
